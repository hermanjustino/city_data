...
df.iloc[651] = df.iloc[651].apply(pd.to_numeric, errors='coerce')
df.loc[438] = pd.to_numeric(df.loc[438], errors='coerce')

# Calculate the rate
df.loc['Rate'] = df.iloc[651].sum() / df.loc[438]
...
plt.title('Rate of greek speakers in Toronto neighborhoods')
